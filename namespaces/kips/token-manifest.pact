(module token-manifest GOVERNANCE
  ...
  (defun hash-contents:string (uri:object{mf-uri} hashes:[string])
    (hash { 'uri: uri, 'data: hashes })
  )

  (defun create-manifest:object{manifest} (uri:object{mf-uri} data:[object{mf-datum}])
    { 
      'uri: uri,
  ...
  (defun verify-manifest:bool (manifest:object{manifest})
    (bind manifest
      { 
        'uri:= uri,
        'data:= data
  ...
  (defun enforce-verify-manifest:bool (manifest:object{manifest})
    (enforce
